...
chatlog['agentname'] = chatlog[chatlog['agentid']=='ROCKETCHAT']['conversationid'].map(dict(zip(c, vals)))
...
dfrocketchat = dfrocketchat[dfrocketchat['agentid']=='ROCKETCHAT']
...
def topbar_values(req_dict):
    ...
    ordcount=ordcount[0]
    Rocketsessioncount = chatlog_[chatlog_['agentid']=='ROCKETCHAT']['conversationid'].nunique()
    dftemp=chatlog_
    ...
    AHT=chatlog_[chatlog_['agentid'] =='ROCKETCHAT'].groupby('conversationid')['Duration'].sum().reset_index()
    ...
    RCCount=chatlog_[chatlog_['agentid']=='ROCKETCHAT']['conversationid'].nunique()
...
def neg_scoretable(req_dict):
    ...
    date=df['date']
    dfrocketchat_converged_=dfrocketchat_converged[dfrocketchat_converged['con_request_time'].dt.day==date]
    negative_review_table=dfrocketchat_converged_[(dfrocketchat_converged_['agent_sentiment']=='Negative') ][['conversationid',
    'agentname',
    'cp_name',
    'product',
    'conversationreq_join',
    'conversationres_processed_join',
...
def new_orderstable(req_dict):
    ...
    date=df['date']
    chatlog_=chatlog[chatlog['con_request_time'].dt.day==date]
    dforder= chatlog_[['conversationid','Duration','valid_order','cp_name','product','agentid']]

    dforder =dforder[(dforder['valid_order']!='' ) & (dforder['valid_order']!='A55' ) &

                     (dforder['valid_order']!='A1' ) &  (dforder['valid_order']!='A20' ) &

                    (dforder['valid_order']!='2-10' ) ]

     

    dfordersummary=dforder.assign(**{'TotalDuration_seconds': dforder.groupby(by=['valid_order'])['Duration'].transform(lambda x: x.sum()),

                        'session_count': dforder.groupby(by=['valid_order'])['conversationid'].transform(lambda x: x.nunique()),

    #                      'productlist': dforder[['valid_order','product']].groupby(['valid_order'])['product'].transform(lambda x: list(x)),

    #                      'cplist': dforder.groupby(['valid_order'])['cp_name'].transform(lambda x: list(x)),

    #                      'agentid': dforder.groupby(['valid_order'])['agentid'].transform(lambda x: list(x)),                                 
    ...
    dfordersummary=dfordersummary[['valid_order','session_count','TotalDuration_seconds','product', 'agentid','cp_name']].drop_duplicates()  #not needed chat_TotalDuration_seconds
    ...
    dfordersummary=dfordersummary.groupby(['valid_order','TotalDuration_seconds','session_count']).agg({
